 ...
 * Porter stemmer
 * 
 * I took the porter stemmer from here and minified it:
 ...
 * against the rules in the original // paper, in // // Porter, 1980, An
 * algorithm for suffix stripping, Program, Vol. 14, // no. 3, pp 130-137, // //
 * see also http://www.tartarus.org/~martin/PorterStemmer
 *  // Release 1 be 'andargor', Jul 2004 // Release 2 (substantially revised) by
 * Christopher McKenzie, Aug 2009
 */
...
BM25.Tokenize = function(text) {
	...
	str = str.toLowerCase().replace(/\W/g, ' ').replace(/\s+/g, ' ').trim().split(' ').map(function(a) { return stemmer(a); });

	// Filter out stopStems
	var out = [];
	for (var i = 0, len = str.length; i < len; i++) {
		if (stopStems.indexOf(str[i]) === -1) {
			out.push(str[i]);
		}
	}
...
/**
 * @param doc
 *            Object Expects this parameter to have an id and body properties.
 */
...
BM25.prototype.search = function(query) {
	...
	// Look at each document in turn. There are better ways to do this with
	// inverted indices.
	...
	for (var j = 0, nDocs = keys.length; j < nDocs; j++) {
		...
		for (var i = 0, len = queryTerms.length; i < len; i++) {
			...
			// The whole term is :
			// IDF * (TF * (k1 + 1)) / (TF + k1 * (1 - b + b * docLength /
			// avgDocLength))
...
/*
 * onmessage = function(e) { var payload = e.data; switch (payload.type) {
 * 
 * case 'index-batch': //payload.data = payload.data.slice(0, 1000); var len =
 * payload.data.length; for (var i in payload.data) { bm.addDocument({id: i,
 * body: payload.data[i]}); if (i % 100 === 0) {
 * postMessage({type:"index-update", data: i/len}); } } bm.updateIdf();
 * postMessage({type:"index-update", data: 1}); break;
 * 
 * case 'search': postMessage({type: "search-results", data:
 * bm.search(payload.data)}); break;
 *  } }
 */
...
var docx = {id: 0, body: "a b c d e a"};
docs.push(docx);
docx = {id: 1, body: "a b c a"};
docs.push(docx);
docx = {id: 2, body: "d e a"};
docs.push(docx);
docx = {id: 3, body: "a b c a"};
docs.push(docx);
docx = {id: 4, body:  "d e c"};
docs.push(docx);
docx = {id: 5, body: "a b c d e a"};
docs.push(docx);
docx = {id: 6, body: "b d a g h"};
docs.push(docx);
docx = {id: 7, body: "a b c d e a f g h"};
docs.push(docx);
